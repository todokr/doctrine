...
別の成果物を出力する「プロセス」に割る。プロセス 1 つが doctrine のタスク 1 つ、PR 1 つ、sub-issue 1 つになる。
計画を立てる上で人の決定が要る事項は、分解の前にまとめて人に問う。
大きすぎる仕事はここで割りきり、人の判断が要る仕事は人が行うプロセスとして識別する
（[PFD による分解](superpowers/specs/2026-09-20-pfd-decomposition-design.md)）
4. **人が PFD をレビューする** — 成果物やプロセスを指してコメントし、差し戻しなら 3 へ戻る／承認なら次へ。
承認されると、プロセスごとに Issue の sub-issue が作られる
...
8. マージによって新たに入力が揃ったプロセスがあれば 5 へ戻る。戻るのに人の操作は要らない。doctrine がマージを検知して
次のタスクを作る。PFD の末端の成果物がマージされたら、Issue は完了である
...
手順 2〜5 と 8 を doctrine の中で担うのが **Intake** である（[Intake の PRD](prd/intake.md)）。

Intake が入るまでは、doctrine の外に置いた試作（`pfd`）がこの流れを担う。試作は GitHub Issue だけを扱い、分解の起動も、
入力が揃ったプロセスをタスクにする操作（`pfd dispatch`）も人が行い、sub-issue は作らない。
...
深刻さの等級は付けない。等級を付けること自体がどこを見るべきかの誘導になり、人が自分で判断する余地を削るため
...
| pfd      | Issue を PFD で分解するスキルと、検証・図の生成・承認の記録・タスクの投入を行う CLI。構想を確かめるための試作であり、doctrine の外に置く。doctrine との接点は `dctl add` と `dctl ls` だけ。構想は回ると分かったので、Intake としてコアとアプリに組み込み、組み込みが済んだら削除する（[Intake の PRD](prd/intake.md)） |
...
デーモンは GitHub 向けの HTTP クライアントもトークンも持たない。
...
- **ステップ** — ワークフロー定義中の1要素。`command` / `agent` / `approval` / `guide` の4種

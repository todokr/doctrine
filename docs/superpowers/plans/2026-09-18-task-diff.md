...
- [ ] **Step 5: `last_review` を拒否している暫定ガードを消す**

#43 がマージされるまでの間、`case "task.diff"` には
`since: "last_review"` を明示的に拒否するガードが入っている
（「まだ実装されていません（#43 待ち）」で `throw` する分岐）。
このタスクはその「まだ」を終わらせる回なので、ガードとそれに対応するテストを
両方消してから Step 6 の実装に進む。

- `src/daemon/handlers.ts` の `case "task.diff"` から
  `if (params.since === "last_review") { throw new Error(...) }` を削除する
  （`params.since !== undefined` のチェックは残す）
- `test/daemon/handlers.test.ts` の
  `test("since: last_review はまだ実装されておらず失敗する", ...)` を削除する
  — Step 2 で足す「差し戻し後、since: last_review は前回レビュー以降の差分だけを
  返す」テストが、この経路が実装されたことをちゃんと検証する
...
Run: `deno test --allow-all test/daemon/handlers.test.ts`
Expected: PASS（新規2件を含む全件。Step 5 で消したテストは無くなっている）

- [ ] **Step 8: 整形・lint・型検査を通してコミット**

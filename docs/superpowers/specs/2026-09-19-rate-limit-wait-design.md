...
- 数値、および数字の文字列（`/^\d+(\.\d+)?$/`）。`1e12` 未満なら秒、以上ならミリ秒として読む。
  実測した形は `"1789828800.0"`（小数付きの epoch 秒の文字列）。claude が `resetsAt` を JSON の
  数値で出すと、`rate_limit_samples.resets_at` は TEXT 列なので、bind した数値がこの文字列に
  なって保存される。つまり実行中のイベントでは数値、DB 経由で読むと小数付きの文字列になる
- 数字と `.` と符号だけでできた、上の形に合わない文字列（`".5"`、`"1.2.3"` など）は読まない。
  V8 の `Date.parse` はこれらを日付として読んでしまう（`".5"` が 2001 年5月になる）ので、
  `Date.parse` に渡す前に弾く
...
当初の `parseResetsAt` は小数付きの epoch 秒の文字列（5.2）を読めなかったので、この代替根拠は
数値の `resetsAt` に対しては「読めない＝待たない」に倒れていた。読めるようにした結果、
同じ状況の結末が `failed` から `rate_limited`（明けるのが6時間より先なら今までどおり `failed`）に変わる。
...
- 上限に当たる前の利用率の見え方は [利用上限の可視化の設計](2026-09-19-rate-limit-visibility-design.md) にある

...
| 項目 | 決定 | 章 |
| --- | --- | --- |
| レールの入口 | 「終了したタスク」の下に「Intake」を足す。`View` に `"intake"` を足す | 3 |
...
| 質問の面 | 1 回分の質問を縦に並べ、全部に答えてから一度に送る。判断材料を選択肢より先に出す。推奨は初めから選ばない | 5 |
...
| 経緯 | どの面からも開ける畳んだ欄。質問と回答・各回の案・コメントと返答・承認・改訂を時系列に読める | 10 |
| 進行中の面 | 「いま自分の番のもの」を先頭に置き、状態で塗った図とプロセスの欄を出す | 8 |
| 改訂中の見せ方 | 状態ではなく、どの面の上にも出る帯と、操作の違いで表す | 8 |
| 全タスクでの見せ方 | サイドバーの行に「Intake #N」の印、見出しに Intake へ戻るリンクを足す | 11 |
| 下書きの永続化 | `drafts.json` の中身を `{ tasks; intakes }` に変える。Rust は変えない | 12 |
| 部品の境界 | 純関数（`pfd.ts` `intake.ts`）と、props だけの表示部品（図・要素の欄・質問・判断材料）に分ける。図と質問はフィクスチャだけで作れる | 13 |
| コアへの追加 | `intake.draft`（案 1 件を中身込みで返す RPC）と `buildAnswerText`（回答の文面の純関数） | 14 |

## 3. 画面の構成とレールの入口（V-1・V-2・V-3・V-4・V-7・C-8）

### 3.1 レールとビュー

- レール（`app/src/components/Sidebar.tsx` の `Rail`）に「Intake」ボタンを、「終了したタスク」の下に足す。
  `View`（`app/src/model.ts`）を `"tasks" | "done" | "intake"` に広げる
...
intakeDrafts: Record<string, IntakeDraft>;   // 12 章
```
...
| `needs_attention` | 要確認の面（`AttentionReason` の種類ごとの説明、「やり直す」、ログ） | 9 |
...
- 「Intake 以外の機能は変わらず動きます」と一文を添える。gh が無いことで、タスクの画面が止まったと誤解させないためである
...
- 開始は `intake.start` を呼ぶ。`alreadyActive` が true なら、「進行中の Intake を開きます」とトーストを出し、その Intake を選ぶ（S-3）。
  成功したら `intakeSel` を返ってきた id にする。コアが失敗にしないのは、既存の id を返すためである（コア設計 13 章）
- S-4（同時に複数）は、画面に制限を置かないことで満たす。S-6（プロセスが 1 つ）は、画面に特別扱いを持たないことで満たす。プロセス 1 つの図もそのまま描ける

## 5. 質問に答える面（Q-2・Q-3・Q-6・Q-7）
...
- 質問 1 件のカードが持つもの: 番号、`prompt`、種類の語（1 つ選ぶ／いくつでも選ぶ／自由記述）、判断材料、選択肢、推奨、「その他」、補足
...
`MaterialView({ material })` は props だけで描く（13 章）。`DiagramView` は store を読まないので、そのまま入れられる。

### 5.2 選択肢と推奨

- `single` はラジオ、`multiple` はチェックで、各選択肢に `label` と `description` を出す。`free` は textarea である
- 推奨の選択肢には「推奨」の印を付け、カードの下に `recommendation.reason` を出す。`free` で推奨があれば、`recommendation.text` を理由とともに見せる
- **推奨を、初めから選んだ状態にしない。** 押すだけで通る形は、推奨を読まずに承認する癖を作るためである。
  レビューアプリ設計 6 章が「見た」印を入れなかった理由と同じである。代わりに、各カードに「推奨を選ぶ」ボタンを置く
...
- 「回答を送る…」で確認のモーダルを開く。モーダルは `buildAnswerText(questions, answers)`（14 章）の文面を `<pre>` で出し、「送る」で `intake.answer` を呼ぶ。
  差し戻しの確認と同じ作法で、送る前に会話へ届く文面を見せる。送れたときだけ下書きを消す（12 章）
- 回答中の入力は下書きに残し、アプリを閉じても消えない

### 5.4 読み返し（Q-7）

過去の質問と回答は、経緯（10 章）でいつでも読める。読むだけの表示は、同じ `QuestionForm` を `readOnly` で使う。
別の表示部品を作らないのは、質問を出したときと読み返すときで見え方が食い違わないようにするためである。

## 6. PFD の図（R-2・R-3・R-4・R-7・V-5・C-3）
...
export function processStages(pfd: Pfd): string[][];                                      // D-2: 段ごとのプロセス id
...
- 図では、プロセスの左上に小さく段の数字「1」「2」…を出す。図の下に「着手の順」として、段ごとのプロセス名を並べる
...
| `ready` | なし | 実線 `--ink-3` | ▷ | 着手可能（`blockedBy` があれば、「改訂中のため止めています」「一時停止中」「sub-issue 待ち」を添える） |
...
- `LOOK` が状態ごとの語・印・CSS クラスを持つ。図と文章版と 8 章の欄が同じ語を使うためである
- 上限待ちと watch の失敗は図に出さない。図の状態はプロセスのものに限る（見張りの失敗は 8 章の箱で出す）
...
- **固定の印（C-3）**: 固定集合はコア設計 6 章のとおり、「`current_task_id` を持つか `human_done_at` のあるプロセス」と、その入力と出力の成果物である。
  ...
  改訂中は、固定集合の要素に「固定」の鍵の印と薄い塗りを付ける。固定の要素にはコメント欄を出さず、「投入済みのため変えられません」と書く
...
- **文章版**: `DiagramView` と同じく `<details>図の中身を文章で読む</details>` を持つ。段の順にプロセスを「入力 → プロセス（担い手・状態の語）→ 出力」で並べる。色に頼らない
...
- 本体: 上に図（6 章）、その右に選んだ要素の欄を置く。欄は、ガイドの欄と同じく sticky にする。
  何も選んでいなければ、計画の題名と goal と「着手の順」（6.4）を出す
...
| 要素 | 出すもの |
| --- | --- |
| 成果物 | 名前・id・`description`・`verify`（確かめ方）・`given` か決定か（決定なら、その質問と回答の文を出す）・作るプロセス・使うプロセス（押すとそのプロセスを選ぶ） |
| プロセス | 名前・id・担い手・`purpose`・`steps`・`done_when`・入力と出力（押すと選ぶ）・段 |

- エージェントのプロセスは、「タスクになったときの prompt」を畳んで置く。開いたときに `intake.processPrompt { intake_id, draft_id, process_id }` を取る。
...
- 下書きは 12 章の `IntakeDraft` である。コメントは `{ target_kind, target_id, body }`（コア設計 13 章の `NewComment`）で持つ。図の印と件数はここから数える
...
- 送っていないコメントがあるときは、ボタンの横に「送っていないコメント N 件は捨てます」と出す。承認では下書きのコメントを送らないためである
...
- 前の案の中身は `intake.draft`（14 章）で取る。`IntakeDetail.drafts` は最新の案だけ中身込みで返すためである

### 7.7 経緯（R-10）

各回の案・コメント・返答は、経緯（10 章）から読める。
...
見出しの下の先頭に、「いま自分の番のもの」の欄を置く（PRD 2.1 の目標 5）。次の 2 種を並べる。

- **`your_turn` のプロセス（H-1・H-2）**: プロセスの目的と完了条件、「決めた内容」の textarea、「完了を記録する」ボタン。
  - 内容は必須で、空白だけは不可である（コアも同じ規則で弾く）。「リポジトリにコミット済みなら、その場所を書けばよい」と添える
  - 記録した内容が下流のタスクの prompt にそのまま載ること（H-3）を、欄に一文で示す
...
- プロセスを選ぶと、7.2 と同じ定義に加えて次を出す。
  - sub-issue のリンク
  - タスク（`task_ids`。今のものを先頭に置き、古いものは「経緯」として並べる）
  - PR のリンクと状態
- タスクを押すと、`view = "tasks"` にしてそのタスクを選ぶ。既存のタスク画面・レビュー画面へ移る
...
- 改訂中（`revising = true`）は、どの面の上にも帯を出す。「改訂中: 自動投入は止まっています。走っているタスクは止めません」と、「改訂をやめる」ボタン（C-5。`intake.abandonRevision`）を持つ
...
- PR には触らないことを、モーダルに明記する（コア設計 13 章）

### 8.7 完了

`completed` では、全プロセスの結果の図と「Issue を閉じる」を出す。Epic を受け入れるかどうかは人の判断であることを、一文で示す（W-11）。
...
- `intake.logs` の末尾。「読み込み直す」ボタンと `intake.updated` で取り直す。ライブ配信は範囲外である（17 章）

### 9.2 要確認の面

`needs_attention` の面である。`AttentionReason` の種類ごとに説明を出す。

| kind | 出すもの |
| --- | --- |
| `agent_failed` | `message` |
| `invalid_output` | 違反（`issues`）の一覧 |
| `wrote_repository` | 書き換えたパス（`paths`）の一覧と、「worktree は戻しました」 |
...
- どの面からも開ける、畳んだ欄「経緯」を置く
- 時系列に次を並べる。
  - 質問のまとまりと回答（5 章の部品を読むだけで使う）
  - 各回の案（押すと、その案の図を読むだけで開く。中身は `intake.draft`）
  - 各回のコメントと返答
  - 承認
  - 改訂の開始
- 改訂の後でも、最初の計画レビューまで遡れる。コアが追記だけの表で持つためである（コア設計 4 章）

## 11. 全タスクでの Intake 由来のタスク（V-6）
...
- Intake 由来のタスクは、通常のタスクとして「全タスク」の区分に並ぶ。Intake をタスクの一覧に混ぜることはしない（PRD 5.2。19 章）
...
| 表示だけの部品 | `PfdDiagram`、`PfdElementPanel`、`QuestionCard` / `QuestionForm`（`{ questions; answers; onChange; readOnly? }`）、`MaterialView`（`{ material }`） | store を読まず props だけで完結する |
...
- **`intake.draft`** — `{ intake_id; draft_id } → { id: number; seq: number; pfd: Pfd; hash: string; replies: CommentReply[]; created_at: string }`。
  コア設計 13 章の `IntakeDetail.drafts` は最新の案だけ中身込みである。次の 3 つは最新でない案の中身を要る。
  - R-7 の、前の案からの差分
  - 改訂中に、承認済みの案と新しい案を並べて見ること
  - R-10 の、経緯で古い案を読み返すこと
...
画面が担わないものは「コア」と書き、コア設計の章を併記する。S の要求は同じ表に入れる。
...
| R-4 | 6.8（印）、7.3（コメント欄）、12 |
| R-5 | 7.4 |
| R-6 | コア（コア設計 7・9）。画面は 7.4 の送信 |
| R-7（S） | 6.7（変化の印）、7.6（返答）、14 |
| R-8 | 7.5（`draft_id` と `hash`） |
| R-9 | 7.5（操作は画面だけ）。保証はコア設計 6 |
| R-10 | 10、14 |
| H-1 | 8.1（あなたの番の欄）、3.1（点） |
| H-2 | 8.1 |
| H-3 | 8.1（示すだけ）。載せるのはコア設計 11.5 |
...
- **Intake をサイドバーのタスクの一覧に混ぜる。** Intake はタスクではない（PRD 5.2）。混ぜると `sel` の意味が崩れる。Intake 由来のタスクだけを通常のタスクとして並べ、印で戻れるようにする（11 章）
- **レールに件数の数字を出す。** 既存のレビュー待ちの点と揃える。件数は `title` に出す（3.1）
- **`IntakeDetail` に全部の案の中身を載せる。** 取り直しのたびに重くなる。`intake.draft` で 1 件ずつ取る（14 章）

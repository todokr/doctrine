...
| 計画レビュー | 図の下に、選んだ要素の欄を置く。要素と全体にコメントを付け、差し戻す・承認する | 7 |
...
- 本体: 上に図（6 章）、その下に選んだ要素の欄を置く。PFD は横に長いので、図に面の幅を全部渡す。
...
- 欄は図の下で面の幅いっぱいに広がるので、広いときは列に分ける。左に本文（成果物の種類・決定・説明・確かめ方、プロセスの目的・手順・完了の条件・プロンプト）、中につながり（前段・後続、入力・出力）と進行中の状態、右にコメントを置く。
  幅が足りなければコメントを中の列の下に回し、さらに狭ければ 1 列に積む。担い手と段は見出しの id の横に並べる

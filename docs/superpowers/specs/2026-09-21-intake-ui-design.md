...
| 過去の質問・案の読み返し | 画面では扱わない（PRD の Q-7・R-10 との差） | 15・17 |
...
Q-7（過去の質問と回答の読み返し）は画面では扱わない（17 章）。
...
| Q-7 | 画面では扱わない（経緯はコアが追記だけの表で保持する。コア設計 4・13）。17 章 |
...
| R-10 | 画面では扱わない（経緯はコアが追記だけの表で保持する。コア設計 4・13）。17 章 |
...
- **過去の質問と回答・各回の案の読み返し（PRD の Q-7・R-10）。** PRD はどちらも M の要求だが、本 spec の画面は満たす手段を持たない。PRD との差である。
  コアは、回答・コメント・案・承認を追記だけの表で持つ（コア設計 4 章）ので、後から読み返す面を足せば満たせる。どの面で、どの形で読み返すかは決めていない。
  決めるときは、PRD を先に直すか、この差を PRD に書き戻す
...
- **経緯の欄を置く。** 判断と操作に要らないものは画面に載せない（1 章）。PRD の Q-7・R-10 との差になる（17 章）
